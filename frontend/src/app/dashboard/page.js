...
function SubmissionSection({ submissions, refreshDashboard }) {
  ...
  return (
    <section className="bg-[#1a1a24]/60 p-6 rounded-xl border border-[#6332ff]/40 shadow-2xl space-y-4">
      <h2 className="text-2xl font-bold text-[#b480ff] flex items-center space-x-2">
        <Zap size={24} /> <span>Quick Launchpad</span>
      </h2>
      {msg && <p className="text-sm text-yellow-400">{msg}</p>}

      {submissions.length === 0 ? (
        <div className="p-4 bg-gray-700/50 rounded-lg text-gray-400 flex items-center space-x-2">
             <AlertTriangle size={20} className="text-yellow-400" />
            {/* Link component used here required the import */}
            <p>No approved images found. Submit a project on the <Link href="/upload" className="text-[#a855f7] hover:underline">Upload</Link> page.</p>
        </div>
      ) : (
        <div className="space-y-3 max-h-64 overflow-y-auto pr-2">
          {submissions.map((sub) => (
            <div key={sub.id} className="flex justify-between items-center bg-[#2d2d3a] p-3 rounded border border-gray-600/50 hover:bg-[#3d3d4a] transition-colors">
              <div>
...
export default function Dashboard() {
  ...
  if (loading) return (
    <div className="min-h-screen flex items-center justify-center bg-[#0a0a0f] text-gray-400">
      <div className="animate-spin text-4xl text-[#b480ff]">🐳</div>
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0f] text-gray-100">
      <main className="p-8 space-y-10">
        ...
        {/* FIX 3: User Instance Summary Section */}
        <section className="space-y-6">
            <h2 className="text-3xl font-bold text-gray-100">Your Containers at a Glance</h2>
        ...
        <section>
          <h2 className="text-2xl mb-4 font-semibold text-[#b480ff] flex items-center space-x-2">
            <LayoutGrid size={20} /> <span>Your Active Instances ({instances.length})</span>
          </h2>
          {instances.length === 0 ? (
            <p className="text-gray-400">You have no active instances. Launch one from the Quick Launchpad above.</p>
          ) : (
