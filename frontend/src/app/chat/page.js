...
export default function UserChatPage() {
    ...
    const setupWebSocket = useCallback(() => {
        ...
        ws.onopen = () => {
            setStatus('Connected. Ready for chat with Admin Support.');
        };

        ws.onmessage = (event) => {
            // Determine if the message is from the user or the support (server)
            const isUserMessage = event.data.startsWith("[USER");
            const sender = isUserMessage ? 'user' : 'support';
            
            setMessages(prev => [...prev, { text: event.data, sender: sender }]);
        };
    ...
    const sendMessage = (e) => {
        ...
        // Frontend displays the message, backend handles the broadcast logic
